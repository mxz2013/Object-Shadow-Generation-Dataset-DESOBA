...
class ARShadowGANModel(DistangleModel):
    def name(self):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            self.visual_names = ['shadowfree_img', 'shadow_img','instance_mask', 'shadow_mask', 'final_refine']
        else:
            self.visual_names = ['final_refine']
    ...
    def forward(self):
        inputATT = torch.cat([self.shadowfree_img, self.instance_mask], 1)
        self.bg_shadow_attention, self.bg_instance_attention = self.netATT(inputATT)
        self.final_residual = self.netMSP(torch.cat([self.shadowfree_img, self.instance_mask, self.bg_shadow_attention, self.bg_instance_attention],1))
    ...
    def backward(self):
        ...
        pred_real = self.netD(self.final_refine)
        self.loss_G_GAN = self.criterionGAN(pred_real, True)
        self.loss += self.loss_G_GAN * self.opt.lambda_GAN
        self.loss_refine = self.MSELoss(self.final_refine, self.shadow_img)
        self.loss += self.loss_refine * self.opt.lambda_I1
        self.loss_bg_shadow_attention = self.MSELoss(self.bg_shadow_attention, self.bg_shadow_mask)
        self.loss_bg_instance_attention = self.MSELoss(self.bg_instance_attention,self.bg_instance_mask)
        self.loss += self.loss_bg_shadow_attention * self.opt.lambda_M1
        self.loss += self.loss_bg_instance_attention * self.opt.lambda_M1
    ...
    def get_current_errors(self):
        ...
        for i in range(nim):
            ...
            RMSE.append(math.sqrt(mean_squared_error(gt, prediction)))
            ...
            prediction_tensor = (getattr(self, 'final_refine').data[i:i + 1, :, :, :]/2 + 0.5) * 255
            mask_tensor = (getattr(self, 'shadow_mask').data[i:i + 1, :, :, :]/2 + 0.5)
            SSIM.append(pytorch_msssim.ssim(gt_tensor, prediction_tensor, data_range=255, size_average=True))
