...
def Bbox_To_Mask_2(rects, instance_mask):
    if torch.cuda.is_available():
        ...
        rect_mask = torch.zeros(instance_mask.size()).cpu()
...
def ROIMask_To_Img(masks, rects, instance_mask):
    ...
    for i in range(rects.size()[0]):
        for j in range(rects.size()[1]):
            rect = rects[i, j]
            mask = (masks[i, j] + 1) / 2
            im_coord, mask_coord, new_mask = paste_mask_in_image(mask, rect, instance_mask.size()[-1],
                                                                 instance_mask.size()[-2], 0, 1)
            im_mask[i, :, im_coord[0]:im_coord[2], im_coord[1]:im_coord[3]] = new_mask[:, :,
                                                                              mask_coord[0]:mask_coord[2],
                                                                              mask_coord[1]:mask_coord[3]]
    return im_mask * 2 - 1
...
def Bbox_regression(shadow_mask, img_wh):
    if torch.cuda.is_available():
        ...
        rect = torch.zeros(shadow_mask.size()[0], 5).cpu()
    # rect = torch.zeros(shadow_mask.size()[0],5).cuda()
    rects = []
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    if netG == 'resnet_9blocks':
        ...
        net = ResnetGenerator_Twobranch(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                        n_blocks=9)
        ...
        net = ResnetGenerator_SelfATT(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                      n_blocks=6)
        ...
        net = Attention_UNet_multiply(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                      n_blocks=9)
    elif netG == 'Attention_Twobranch_UNet':
        net = Attention_Twobranch_UNet(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                       n_blocks=9)
    elif netG == 'Attention_Twobranch_UNet_Noshare':
        net = Attention_Twobranch_UNet_Noshare(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                               n_blocks=9)
        ...
        net = Adain_Mask_Generator(nf=ngf, input_dim_c=input_nc + 1, input_dim_s=input_nc, output_dim=output_nc,
                                   nf_mlp=256, down_class=4, down_content=3, n_mlp_blks=3, n_res_blks=2, latent_dim=64)
    elif netG == 'Attention_Twobranch_UNet_Bbox':
        net = Attention_Twobranch_UNet_Bbox(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                            n_blocks=9)
    elif netG == 'Attention_Twobranch_UNet_Noshare_bbox':
        net = Attention_Twobranch_UNet_Noshare_bbox(input_nc, output_nc, ngf, norm_layer=norm_layer,
                                                    use_dropout=use_dropout, n_blocks=9)
    elif netG == 'Attention_Twobranch_UNet_Bbox_TwoDecoder':
        net = Attention_Twobranch_UNet_Bbox_TwoDecoder(input_nc, output_nc, ngf, norm_layer=norm_layer,
                                                       use_dropout=use_dropout, n_blocks=9)
    elif netG == 'Attention_Twobranch_UNet_Noshare_bbox_TwoDecoder':
        net = Attention_Twobranch_UNet_Noshare_bbox_TwoDecoder(input_nc, output_nc, ngf, norm_layer=norm_layer,
                                                               use_dropout=use_dropout, n_blocks=9)
    elif netG == 'Attention_UNet_MaskBox_Multitask':
        net = Attention_UNet_MaskBox_Multitask(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                               n_blocks=9)
    elif netG == 'Attention_UNet_MaskBox_ROIMaskRCNN':
        net = Attention_UNet_MaskBox_ROIMaskRCNN(input_nc, output_nc, ngf, norm_layer=norm_layer,
                                                 use_dropout=use_dropout, n_blocks=9)
    elif netG == 'Attention_MaskBox_ROIMaskRCNN':
        net = Attention_MaskBox_ROIMaskRCNN(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                            n_blocks=9)
    elif netG == 'STN':
...
class Attention_UNet_MaskBox_Multitask(nn.Module):
    ...
    def forward(self, input_img, fo_mask, bg_pure_mask, bg_shadow_mask, bg_instance_mask, instance_rects, shadow_rects):
        ...
        if torch.cuda.is_available():
            ...
            shadow_bbox_mask_pred = torch.zeros(fo_mask.size()).cpu()
