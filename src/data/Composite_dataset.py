...
def generate_training_pairs(
    ...
    for pixel in all_instance_pixels:
        instance_ratio = np.sum(instance_mask[instance_mask == pixel] / 255) / np.sum(
            np.ones(np.shape(instance_mask))
        )
    ...
    for i in range(1, object_num):
        ...
        for combine in combines:
            ...
            # fg_object size
            if (
                np.sum(fg_instance / 255) / np.sum(np.ones(np.shape(fg_instance)))
            ) < 0.05:
                continue
...
class CompositeDataset(BaseDataset):
    def initialize(self, opt):
        ...
        for im in self.imname_total:
            instance = Image.open(os.path.join(self.dir_bg_instance, im[0])).convert(
                "L"
            )
    ...
    def __getitem__(self, index):
        ...
        for index_bg in range(len(self.birdy_shadowimg_whole)):
            ...
            bg_instance = cv2.dilate(
                bg_instance, np.ones((10, 10), np.uint8), iterations=1
            )
            ...
            # # two foreground
            if (
                fg_instance_area / bg_instance_areas[-1] > 2.4
                or fg_instance_area / bg_instance_areas[-1] < 1.2
            ):
                continue
            ...
            ratio = np.sum(np.array(birdy["instancemask"]) / 255) / np.sum(
                birdy["max_object"] / 255
            )
            birdy["ratio"] = ratio
            ...
            if (
                np.sum(birdy["pure_bg_area"])
                / np.sum(np.ones(np.shape(birdy["pure_bg_area"])))
                < 0.3
            ):
                # print('continue')
                continue
            ...
            for i in range(self.opt.loadSize - 1 * pixel_th, 1 * pixel_th, -interval):
                for j in range(
                    self.opt.loadSize - 1 * pixel_th, 1 * pixel_th, -interval
                ):
                    ...
                    new_foreground_object_mask = np.zeros(
                        np.shape(birdy["bg_instance"])
                    )
                    new_foreground_shadow_mask = np.zeros(
                        np.shape(birdy["bg_instance"])
                    )

                    t1 = new_foreground_object_mask[
                        x_offset : x_offset + col_length,
                        y_offset : y_offset + row_length,
                    ]
                    t11 = fg_instance[min(x) : max(x), min(y) : max(y)]
                    t2 = new_foreground_shadow_mask[
                        x_offset : x_offset + col_length,
                        y_offset : y_offset + row_length,
                    ]
                    ...
                    new_foreground_object_mask[
                        x_offset : x_offset + col_length,
                        y_offset : y_offset + row_length,
                    ] = fg_instance[min(x) : max(x), min(y) : max(y)]
                    new_foreground_shadow_mask[
                        x_offset : x_offset + col_length,
                        y_offset : y_offset + row_length,
                    ] = fg_shadow[min(x) : max(x), min(y) : max(y)]
                    new_foreground_object_mask[new_foreground_object_mask > 0] = 255
                    new_foreground_shadow_mask[new_foreground_shadow_mask > 0] = 255

                    new_foreground_object[
                        x_offset : x_offset + col_length,
                        y_offset : y_offset + row_length,
                        :,
                    ] = A_img_foreground[min(x) : max(x), min(y) : max(y), :]
                    ...
                    if np.sum(overlap) == np.sum(new_foreground_object_mask):
                        ...
                        birdy_new["instancemask"] = Image.fromarray(
                            np.uint8(new_foreground_object_mask), mode="L"
                        )
                        # records.append(new_foreground_object_mask)
                        # fg_shadow = Image.fromarray(np.uint8(new_foreground_shadow_mask), mode='L')
                        birdy_new["B"] = Image.fromarray(
                            np.uint8(new_foreground_shadow_mask), mode="L"
                        )
                        ####replacing the area of foreground
                        final_img = new_foreground_object * (
                            np.tile(
                                np.expand_dims(new_foreground_object_mask / 255, -1),
                                (1, 1, 3),
                            )
                        ) + birdy["A"] * (
                            1
                            - np.tile(
                                np.expand_dims(
                                    np.array(new_foreground_object_mask) / 255, -1
                                ),
                                (1, 1, 3),
                            )
                        )
                        birdy_new["C"] = Image.fromarray(
                            np.uint8(final_img), mode="RGB"
                        )
