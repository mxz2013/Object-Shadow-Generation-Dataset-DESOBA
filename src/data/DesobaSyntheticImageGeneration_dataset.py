...
def Generation_Synthetic_Composite_Image(
    birdy_deshadoweds,
    birdy_shadoweds,
    birdy_fg_instances,
    birdy_fg_shadows,
    ...
    for i in range(1, object_num):
        ...
        for combine in selected_instance_pixel_combine:
            ...
            synthetic_composite_image = deshadowed_image * (
                np.tile(np.expand_dims(np.array(fg_shadow_new) / 255, -1), (1, 1, 3))
            ) + shadow_image * (
                1
                - np.tile(np.expand_dims(np.array(fg_shadow_new) / 255, -1), (1, 1, 3))
            )
            synthetic_composite_image = Image.fromarray(
                np.uint8(synthetic_composite_image), mode="RGB"
            )
...
class DesobaSyntheticImageGenerationdataset(BaseDataset):
    def initialize(self, opt):
        ...
        for imname_list in self.imname:
            ...
            instance = (
                Image.open(os.path.join(self.dir_instance, imname))
                .convert("L")
                .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
            )
            shadow = (
                Image.open(os.path.join(self.dir_shadow, imname))
                .convert("L")
                .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
            )
            ...
            (
                ...
                self.bg_instance_masks,
                self.bg_shadow_masks,
            ) = Generation_Synthetic_Composite_Image(
                self.synthetic_composite_imgs,
                self.shadow_imgs,
                self.fg_instance_masks,
                self.fg_shadow_masks,
