...
class ShadowGenerationDatasetInferencedataset(BaseDataset):
    def initialize(self, opt):
        self.opt = opt
        self.is_train = self.opt.isTrain
        self.root = opt.dataroot
        self.dir_shadowimg = opt.shadowimg_path  # os.path.join(opt.dataroot, 'A')
        self.dir_shadowfree = (
            opt.shadowfree_path
        )  # os.path.join(opt.dataroot, opt.phase + 'C')
        self.dir_fg_instance = opt.instance_path
        self.dir_fg_shadow = opt.shadowmask_path
        ...
        for root, _, fnames in sorted(os.walk(self.dir_shadowimg)):
            ...
            for name in sorted(fname_int, key=int):
                ...
                if fname.endswith(".png"):
                    X = dict()
                    X["shadowimg_path"] = os.path.join(root, fname)
                    X["shadowfree_path"] = os.path.join(self.dir_shadowfree, fname)
                    X["fginstance_path"] = os.path.join(self.dir_fg_instance, fname)
                    X["fgshadow_path"] = os.path.join(self.dir_fg_shadow, fname)
                    X["bginstance_path"] = os.path.join(self.dir_bg_instance, fname)
                    X["bgshadow_path"] = os.path.join(self.dir_bg_shadow, fname)

                    shadowimg = (
                        Image.open(X["shadowimg_path"])
                        .convert("RGB")
                    ...
                    instance = (
                        Image.open(X["fginstance_path"])
                        .convert("L")
                        .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
                    )
                    shadow = (
                        Image.open(X["fgshadow_path"])
                        .convert("L")
                        .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
                    )
                    bg_instance = (
                        Image.open(X["bginstance_path"])
                        .convert("L")
                        .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
                    )
                    bg_shadow = (
                        Image.open(X["bgshadow_path"])
                        .convert("L")
                        .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
                    )

                    self.birdy_deshadoweds.append(shadowfree)
                    self.birdy_shadoweds.append(shadowimg)
                    self.birdy_fg_instances.append(instance)
                    self.birdy_fg_shadows.append(shadow)
                    self.birdy_bg_instances.append(bg_instance)
                    self.birdy_bg_shadows.append(bg_shadow)
    ...
    def __getitem__(self, index):
        ...
        birdy["light_image"] = birdy["instancemask"]
        birdy["edge"] = birdy["instancemask"]
    ...
    def name(self):
        return "ShadowGenerationDatasetInference"
