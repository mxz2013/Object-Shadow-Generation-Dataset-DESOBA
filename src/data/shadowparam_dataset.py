...
def generate_training_pairs(
        ...
        birdy_edges,
        birdy_shadowparas,
        birdy_shadow_object_ratio,
        birdy_instance_boxes,
        birdy_shadow_boxes,
        birdy_instance_box_areas,
        birdy_shadow_box_areas,
        birdy_im_lists,
):
    ...
    for i in range(1, object_num):
        ...
        for combine in selected_instance_pixel_combine:
            ...
            fg_shadow_dilate = cv2.dilate(
                fg_shadow, np.ones((10, 10), np.uint8), iterations=1
            )
            fg_shadow_erode = cv2.erode(
                fg_shadow, np.ones((10, 10), np.uint8), iterations=1
            )
            ...
            if len(instance_pixels) == 1:
                fg_shadow_new = cv2.dilate(
                    fg_shadow, np.ones((20, 20), np.uint8), iterations=1
                )
                ...
                fg_shadow_new = cv2.dilate(
                    fg_shadow, np.ones((5, 5), np.uint8), iterations=1
                )
            ...
            # birdy_fg_instances = np.append(birdy_fg_instances, fg_instance)
            # birdy_fg_shadows = np.append(birdy_fg_shadows, fg_shadow)
            # birdy_instance_boxes = np.append(birdy_instance_boxes, torch.IntTensor(np.array(fg_instance_boxes)))
            # birdy_shadow_boxes = np.append(birdy_shadow_boxes, torch.IntTensor(np.array(fg_shadow_boxes)))
            # birdy_im_lists = np.append(birdy_im_lists, imname_list)
            ...
            for i in range(len(fg_instance_boxes)):
                fg_instance_box_areas = bbox_to_mask(
                    fg_instance_boxes[i], fg_instance_box_areas
                )
                fg_shadow_box_areas = bbox_to_mask(
                    fg_shadow_boxes[i], fg_shadow_box_areas
                )
            fg_instance_box_areas = Image.fromarray(
                np.uint8(fg_instance_box_areas), mode="L"
            )
            fg_shadow_box_areas = Image.fromarray(
                np.uint8(fg_shadow_box_areas), mode="L"
            )
            ...
            birdy_deshadoweds.append(
                                          Image.fromarray(np.uint8(new_shadow_free_image), mode="RGB")
                                          )
            ...
            birdy_shadow_object_ratio = np.append(birdy_shadow_object_ratio, shadow_object_ratio)
            fg_instance = []
            fg_shadow = []
            bg_instance = []
            bg_shadow = []
...
class ShadowParamDataset(BaseDataset):
    def initialize(self, opt):
        ...
        self.imname = []
        if self.is_train:
            for f in open(os.path.join(opt.dataroot, "Training_labels.txt")):
                self.imname.append(f.split())
        else:
            for f in open(os.path.join(opt.dataroot, "Testing_labels.txt")):
                self.imname_total.append(f.split())

            for im in self.imname_total:
                instance = Image.open(
                    os.path.join(self.dir_bg_instance, im[0])
                ).convert("L")
                instance = np.array(instance)
                instance_pixels = np.unique(np.sort(instance[instance > 0]))
                shadow = Image.open(os.path.join(self.dir_bg_shadow, im[0])).convert(
                    "L"
                )
        ...
        # self.birdy_deshadoweds = np.array([])
        # self.birdy_shadoweds = np.array([])
        # self.birdy_fg_instances = np.array([])
        # self.birdy_fg_shadows = np.array([])
        # self.birdy_bg_instances = np.array([])
        # self.birdy_bg_shadows = np.array([])
        # self.birdy_edges = np.array([])
        # self.birdy_shadow_params = np.array([])
        # self.birdy_shadow_object_ratio = np.array([])
        # self.birdy_instance_boxes = np.array([])
        # self.birdy_shadow_boxes = np.array([])
        # self.birdy_instance_box_areas = np.array([])
        # self.birdy_shadow_box_areas = np.array([])
        # self.birdy_imlists = np.array([])
        ...
        for id, imname_list in enumerate(self.imname[:]):
            ...
            A_img = (
                ...
                .convert("RGB")
                .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
            )
            C_img = (
                Image.open(os.path.join(self.dir_C, imname))
                .convert("RGB")
                .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
            )
            new_mask = (
                Image.open(os.path.join(self.dir_new_mask, imname))
                .convert("L")
                .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
            )
            instance = (
                Image.open(os.path.join(self.dir_bg_instance, imname))
                .convert("L")
                .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
            )
            shadow = (
                Image.open(os.path.join(self.dir_bg_shadow, imname))
                .convert("L")
                .resize((self.opt.loadSize, self.opt.loadSize), Image.NEAREST)
            )
            imlist = imname_list
