...
class ShadowParamDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if is_samebg_difffg:
            ...
            birdy["bg_shadow"] = Image.fromarray(
                np.uint8(
                    np.array(self.birdy_bg_shadows[index_bg])
                    + np.array(self.birdy_fg_shadows[index_bg])
                ),
                mode="L",
            )
            birdy["bg_instance"] = Image.fromarray(
                np.uint8(
                    np.array(self.birdy_bg_instances[index_bg])
                    + np.array(self.birdy_fg_instances[index_bg])
                ),
                mode="L",
            )
            ...
            birdy["bg_shadow"] = Image.fromarray(
                np.uint8(
                    np.array(self.birdy_bg_shadows[index])
                    + np.array(self.birdy_fg_shadows[index])
                ),
                mode="L",
            )
            birdy["bg_instance"] = Image.fromarray(
                np.uint8(
                    np.array(self.birdy_bg_instances[index])
                    + np.array(self.birdy_fg_instances[index])
                ),
                mode="L",
            )
            birdy["light_image"] = birdy["A"]
        ...
        for i in range(self.opt.loadSize - 20, 0, -1):
            ...
            x_offset = int(
                (self.opt.loadSize - col_length - 1) / self.opt.loadSize * (i + 10)
            )
            y_offset = int(
                (self.opt.loadSize - row_length - 1) / self.opt.loadSize * (i + 10)
            )
            ...
            new_foreground_object_mask[
                x_offset : x_offset + col_length, y_offset : y_offset + row_length
            ...
            if np.sum(overlap) == np.sum(new_foreground_object_mask):
                ...
                final_img = new_foreground_object * (
                    np.tile(
                        np.expand_dims(new_foreground_object_mask / 255, -1), (1, 1, 3)
                    )
                ) + birdy["A"] * (
                    1
                    - np.tile(
                        np.expand_dims(np.array(new_foreground_object_mask) / 255, -1),
                        (1, 1, 3),
                    )
                )
